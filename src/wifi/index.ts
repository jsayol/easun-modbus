...
import { instance as mockSerialPort } from "./mock-serialport";
...
let easunWifiSocket: net.Socket;
let udpSocket: dgram.Socket;
let tcpModbusCounter = 1;
let waitTimeout: ReturnType<typeof setTimeout>;
let keepaliveInterval: ReturnType<typeof setInterval>;

function setupEasunWifiServer(wifiIP: string, localIP: string): Promise<void> {
    return new Promise((resolve, reject) => {
        createServer(wifiIP, localIP, resolve, reject);
    });
}

function createServer(wifiIP: string, localIP: string, resolve: (value?: any) => void, reject: (reason?: any) => void) {
...
async function sendUDPConnectCommand(wifiIP: string, localIP: string, reject: (reason?: any) => void): Promise<void> {
...
async function sendUDP(data: string | Buffer, address: string): Promise<number> {
    return new Promise((resolve, reject) => {
        console.log("[UDP][SEND]", data);

        if (!udpSocket || !(udpSocket instanceof dgram.Socket)) {
...
export async function setupEasunWifiConnection(wifiIP: string, localIP: string): Promise<void> {
    await setupEasunWifiServer(wifiIP, localIP);

    mockSerialPort.on("write", async (data) => {
        console.log("[ON WRITE]", data)
        if (typeof data === "string") {
            data = Buffer.from(data);
        }
    ...
    easunWifiSocket.on("data", data => {
        ...
        if (counter !== 0) {
            mockSerialPort._forwardData(data);
        }
    });
}